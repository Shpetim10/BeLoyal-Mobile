...
class CustomerViewAllTransactionsPage extends StatefulWidget {
  ...
  @override
  State<CustomerViewAllTransactionsPage> createState() => _CustomerViewAllTransactionsPageState();
}

class _CustomerViewAllTransactionsPageState extends State<CustomerViewAllTransactionsPage>
  ...
  List<MockTransaction> _filtered(String typeFilter) {
    final source = [...(widget.transactions ?? CustomerMockData.transactions)]
  ...
  Map<String, List<MockTransaction>> _grouped(List<MockTransaction> txs) {
  ...
  Widget build(BuildContext context) {
    final allTxs = _filtered('ALL');
    final totalEarned = allTxs.where((t) => t.type == 'EARN').fold(0, (s, t) => s + t.points);
    final totalSpent = allTxs.where((t) => t.type == 'REDEEM').fold(0, (s, t) => s + t.points.abs());
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            ...
            Text('${allTxs.length} transactions', style: AppTypography.dmSans(fontSize: 12, color: AppColors.textMutedDark)),
          ],
        ),
      ),
      body: Column(
        children: [
          ...
          Padding(
            ...
            child: Row(
              children: [
                _SummaryPill(label: 'Earned', value: '+$totalEarned pts', color: AppColors.success),
                const SizedBox(width: 10),
                _SummaryPill(label: 'Spent', value: '-$totalSpent pts', color: AppColors.error),
                const SizedBox(width: 10),
                _SummaryPill(label: 'Total', value: '${allTxs.length}', color: AppColors.primary),
              ],
            ),
          ),
          // Search
          ...
          Container(
            ...
            child: TabBar(
              ...
              labelStyle: AppTypography.dmSans(fontSize: 13, fontWeight: FontWeight.w700),
              unselectedLabelStyle: AppTypography.dmSans(fontSize: 13, fontWeight: FontWeight.w500),
              tabs: const [
                Tab(text: 'Total'),
                Tab(text: 'Earned'),
                Tab(text: 'Spent'),
                ...
                if (txs.isEmpty) {
                  return Center(
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.receipt_outlined, size: 56, color: AppColors.textMutedDark.withValues(alpha: 0.3)),
                        const SizedBox(height: 16),
                        Text('No transactions found.', style: AppTypography.dmSans(fontSize: 14, color: AppColors.textMutedDark)),
                      ],
                    ),
                  );
                }
                        ...
                        Padding(
                          ...
                          child: Row(
                            children: [
                              ...
                              Text(
                                entry.key,
                                style: AppTypography.dmSans(fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.textMutedDark),
                              ),
                            ],
                          ),
                        ),
                        ...entry.value.map((tx) => _TxCard(tx: tx, timeFmt: dateFmt)),
                      ],
                    );
                  }).toList(),
                );
  ...
  Widget build(BuildContext context) {
    return Expanded(
      child: Container(
        ...
        child: Column(
          children: [
            Text(value, style: AppTypography.dmMono(fontSize: 13, fontWeight: FontWeight.w700, color: color)),
            const SizedBox(height: 2),
            Text(label, style: AppTypography.dmSans(fontSize: 10, color: AppColors.textMutedDark)),
          ],
        ),
      ),
    );
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Center(child: Text(tx.logoEmoji, style: const TextStyle(fontSize: 20))),
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 2),
                Text(tx.description, style: AppTypography.dmSans(fontSize: 11, color: AppColors.textMutedDark, height: 1.3), maxLines: 1, overflow: TextOverflow.ellipsis),
                    ...
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Text(_typeLabel, style: AppTypography.dmSans(fontSize: 9, fontWeight: FontWeight.w700, color: _color)),
                        ],
                      ),
                    ),
                    const SizedBox(width: 8),
                    Text(timeFmt.format(tx.date), style: AppTypography.dmSans(fontSize: 10, color: AppColors.textMutedDark)),
                    if (tx.billAmount > 0) ...[
                      const SizedBox(width: 6),
                      Text('· L ${tx.billAmount.toStringAsFixed(0)}', style: AppTypography.dmMono(fontSize: 10, color: AppColors.textMutedDark)),
                    ],
                  ],
                ),
              ],
          ...
          Column(
            ...
            children: [
              Text(
                ...
                style: AppTypography.dmMono(fontSize: 17, fontWeight: FontWeight.w700, color: _color),
              ),
