...
class CouponExpiredState {
  const CouponExpiredState({
    this.coupons = const [],
...
class CouponExpiredController extends Notifier<CouponExpiredState> {
  @override
  CouponExpiredState build() => const CouponExpiredState();
  ...
  Future<void> fetchCoupons(int businessId, {bool reset = true}) async {
    ...
    try {
      ...
      final result = await _repo.listExpiredCoupons(
...
final couponExpiredControllerProvider =
    NotifierProvider<CouponExpiredController, CouponExpiredState>(
      CouponExpiredController.new,
    );
